...
module.exports = {
    ...
    buildMedication: function(medication, array) {
        ...
        if (medication.medicine.hasOwnProperty('code')) {
            medication.medicine.code.forEach((entry) => {
                array.push('<mmlPs:code mmlPs:system='); // 正しい
                // array.push('<mmlPs:code system=');  // 試験
        ...
        // 総投与回数
        if (medication.hasOwnProperty('repetitions')) {
            array.push('<mmlPs:repetitions>');
            array.push(medication.repetitions);
            array.push('</mmlPs:repetitions>');
        }
    ...
    // PrescriptionのMML
    build: function(prescription, array) {
        // PrescriptionModule
    ...
    deleteInstance: function (arr) {
        arr.push('<mmlPs:PrescriptionModule/>');
    }
};
