...
module.exports = {
    ...
    buildAccessRight: function(target) {
        var arr = [];
        arr.push('<accessRight');
        arr.push(' permit=');
    ...
    buildSecurityLevel: function(target) {
        var arr = [];
        arr.push('<securityLevel>');
        target.forEach((entry) => {
            arr.push(this.buildAccessRight(entry));
        });
        arr.push('</securityLevel>');
        return arr.join('');
    },

    buildTitle: function(target) {
        var arr = [];
        arr.push('<title');
    ...
    buildConfirmDate: function(target) {
        var arr = [];
        arr.push('<confirmDate');
        if (target.hasOwnProperty('attr')) {
            if (target.attr.hasOwnProperty('start')) {
                arr.push(' start=');
                arr.push(util.addQuote(target.attr.start));
            }
            if (target.attr.hasOwnProperty('end')) {
                arr.push(' end=');
                arr.push(util.addQuote(target.attr.end));
            }
    ...
    buildExtRefs: function(target) {
        var arr = [];
        arr.push('<extRefs>');
        target.forEach((entry) => {
            arr.push(commonBuilder.buildExtRef(entry));
        });
        arr.push('</extRefs>');
        return arr.join('');
    },
    ...
    build: function(target) {
        ...
        arr.push('>');
        arr.push(this.buildSecurityLevel(target.securityLevel));
        arr.push(this.buildTitle(target.title));
        arr.push(this.buildDocId(target.docId));
        arr.push(this.buildConfirmDate(target.confirmDate));
        arr.push(commonBuilder.buildCreatorInfo(target.CreatorInfo));
        arr.push(this.buildExtRefs(target.extRefs));
        arr.push('</docInfo>');
        return arr.join('');
    }
};
