...
var surgeryItem = {
    surgicalInfo: {                                     // 手術ヘッダー情報
        ...
        surgicalDepartment: [],                         // 手術実施診療科情報 ? [mmlDp:Department]
        patientDepartment: []                           // 手術時に患者の所属していた診療科 ? [mmlDp:Department]
    },
    surgicalDiagnosis: [],                              // 外科診断情報 [mmlRd:RegisteredDiagnosisModule]
    surgicalProcedure: [],                              // 手術法情報 [procedureItem]
    surgicalStaffs: [],                                 // 麻酔を除く手術スタッフの情報 ? [staff]
    anesthesiaProcedure: [],                            // 麻酔法名情報 ? [title]
...
var procedureItem = {
    ...
    procedureMemo: ''                                   // 手術法に関する追加事項 choice ?
};
...
var staff = {
    ...
    staffInfo: []                                       // スタッフ ID 情報 [mmlPsi:PersonalizedInfo]
};
*******************************************************************************/
...
module.exports = {
    ...
    buildSurgicalInfo: function (target, arr) {
        ...
        if (target.hasOwnProperty('surgicalDepartment')) {
            ...
            // Array
            target.surgicalDepartment.forEach((e) => {
                commonBuilder.buildDepartment(e, arr);
            });
            arr.push('</mmlSg:surgicalDepartment>');
        }
        ...
        if (target.hasOwnProperty('patientDepartment')) {
            ...
            // Array
            target.patientDepartment.forEach((e) => {
                commonBuilder.buildDepartment(e, arr);
            });
            arr.push('</mmlSg:patientDepartment>');
        }
    ...
    buildSurgicalStaffs: function (target, arr) {
        ...
        target.forEach((entry) => {
            ...
            arr.push('>');
            entry.staffInfo.forEach((e) => {
                // console.log(JSON.stringify(e, null, 4));
                arr.push('<mmlSg:staffInfo>');
                commonBuilder.buildPersonalizedInfo(e, arr);
                arr.push('</mmlSg:staffInfo>');
            });
            arr.push('</mmlSg:staff>');
        });
    ...
    buildAnesthesiaProcedure: function (target, arr) {
        ...
        target.forEach((entry) => {
            arr.push('<mmlSg:title');
            if (entry.hasOwnProperty('attr') && entry.attr.hasOwnProperty('code')) {
                arr.push(' mmlSg:code=');
                arr.push(utils.addQuote(entry.attr.code));
            }
            if (entry.hasOwnProperty('attr') && entry.attr.hasOwnProperty('system')) {
                arr.push(' mmlSg:system=');
                arr.push(utils.addQuote(entry.attr.system));
            }
            arr.push('>');
    ...
    buildReferenceInfo: function (target, arr) {
        ...
        // });
        commonBuilder.buildExtRef(target, arr);
        arr.push('</mmlSg:referenceInfo>');
    },
    ...
    build: function (target, arr) {
        ...
        target.surgeryItem.forEach((entry) => {
            ...
            // surgicalStaffs
            if (entry.hasOwnProperty('surgicalStaffs')) {
                // console.log(JSON.stringify(entry.surgicalStaffs, null, 4));
                this.buildSurgicalStaffs(entry.surgicalStaffs, arr);
            }

            // anesthesiaProcedure
            if (entry.hasOwnProperty('anesthesiaProcedure')) {
                this.buildAnesthesiaProcedure(entry.anesthesiaProcedure, arr);
            }

            // anesthesiologists
            if (entry.hasOwnProperty('anesthesiologists')) {
                this.buildAnesthesiologists(entry.anesthesiologists, arr);
            }

            // anesthesiaDuration
            if (entry.hasOwnProperty('anesthesiaDuration')) {
                this.buildAnesthesiaDuration(entry.anesthesiaDuration, arr);
            }

            // operativeNotes
            if (entry.hasOwnProperty('operativeNotes')) {
                this.buildOperativeNotes(entry.operativeNotes, arr);
            }

            // referenceInfo
            if (entry.hasOwnProperty('referenceInfo')) {
