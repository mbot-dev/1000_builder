...
module.exports = {
    ...
    buildExtRef: function(target, arr) {
        ...
        // ここでFileを生成する base64 to File with href
        if (target.attr.hasOwnProperty('base64')) {
            var buf = new Buffer(target.attr.base64, 'base64');
            var path = config.mmlOutput.path + '/' + target.attr.href;
