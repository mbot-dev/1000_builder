...
千年ビルダーは、[千年カルテプロジェクト](https://www.facebook.com/gEHR-398609153661839/)へ参加するためにカルテのデータを [MML4.0](http://www.medxml.net/MML40j/mml4.html) にエンコードするサーバーアプリケーションです。利用にあたってMMLの知識は不要です。
...
この時の関係（患者、医師、処方）を単純なJSONデータで表し、サーバーへポストします。（個人名: [なんちゃって個人情報](http://kazina.com/dummy/) を使用）
...
サーバーは受け取ったデータをMML4.0に変換しレスポンスします。このXMLインスタンスを1000年プロジェクトに送信します。デフォルトで次の権限が設定されるようにしています。

 * 診察したクリニック: 参照、修正、削除する全権限
 * 患者: 参照のみ
 * 患者が受診したことのある病院: 参照可
...
* 検査についても単純なJSONデータをポストします。新たに検査会社の情報が必要になります。
