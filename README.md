...
千年ビルダーは、[千年カルテプロジェクト](https://www.facebook.com/gEHR-398609153661839/)へ参加するためにカルテのデータを [MML4.0](http://www.medxml.net/MML40j/mml4.html) にエンコードするサーバーアプリケーションです。しかし利用にあたってMMLの知識は不要です。処方、検査、病名のデータを単純な構造で表し、サーバーへポストするだけです。
...
この時、患者、医師、処方を下記のような単純な構造のデータにし、サーバーへポストします。（[なんちゃって個人情報](http://kazina.com/dummy/) を使用）
...
* サーバーとクライアントは JSON-RPC 2.0 のリモートプロシジャーコールで通信します。
