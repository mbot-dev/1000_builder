...
1000年ビルダーは、[千年カルテプロジェクト](https://www.facebook.com/gEHR-398609153661839/)へ参加するためにカルテのデータを [MML4.0](http://www.medxml.net/MML40j/mml4.html) にエンコードするサーバーアプリケーションです。しかし利用にあたってMMLの知識は不要です。処方、検体検査、病名を非常に簡単なデータ構造にし、サーバーへポストするだけで済みます。
...
この時、患者、医師、処方を下記のような単純な構造のデータにし、サーバーへポストします。(個人名は[なんちゃって個人情報](http://kazina.com/dummy/) を使用。)
...
サーバーは受け取ったデータをMML4.0形式に変換し次のようなレスポンスを返します。このインスタンスを1000年プロジェクトに送信します。
...
* npm install
* node app.js（JSON-RPC サーバーが起動します）
